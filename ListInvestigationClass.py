...
class ListInvestigation():
    def __init__(self, listOfExperiments :list) -> None: #COUOCU list instead of DynamicExperiment
        self.listOfExperiments = listOfExperiments
        self.nbrExperiments = len(listOfExperiments)
    ...
    def plot_All_noisy_mean(self):
        """Plot all experiments in the list, containing Noise"""
        # compute noise mean values
        for element in range(self.nbrExperiments):
            self.listOfExperiments[element].get_all_magnetic_Field_mean_values_with_noise()

        # create figure
        fig, ax = plt.subplots(figsize=set_size())

        for element in range(self.nbrExperiments):
            ax.plot(self.listOfExperiments[element].magnetic_Field_mean_values_with_noise.keys(), self.listOfExperiments[element].magnetic_Field_mean_values_with_noise.values(), marker = ".", label = self.listOfExperiments[element].name)
        ax.legend()
    ...
    def scale_Experiments_to_Current(self, scaleToCurrent:float):
        self.scaleToCurrent = scaleToCurrent
        for experiment in range(self.nbrExperiments):
            self.listOfExperiments[experiment].scaleToCurrent = scaleToCurrent
            currentScaleFactor = scaleToCurrent / self.listOfExperiments[experiment].dataMeasurement.get_all_Physic_Mean_with_Noise_Values()['V0-Courant-I']
            self.listOfExperiments[experiment].currentScaleFactor = currentScaleFactor
            print(self.listOfExperiments[experiment].currentScaleFactor)
            self.listOfExperiments[experiment].scale_Current_with_currentScaleFactor()

    def plot_scaled_differential_Mean_without_Noise_with_Reference_to_input(self, experimentName:str) -> list:
        """Plot differntial value compared to the passed experiment"""
        # get mean values without noise for ref experiment:
        for experiment in range(self.nbrExperiments):
            if self.listOfExperiments[experiment].name == experimentName:
                reference = self.listOfExperiments[experiment].physic_mean_value_dict_scaled_to_current
                sensorList = self.listOfExperiments[experiment].sensorsOfInterest
            else: pass
        ...
        for experiment in range(self.nbrExperiments):
            if self.listOfExperiments[experiment].name != experimentName:
                listOfNames.append(self.listOfExperiments[experiment].name)
                faulty = self.listOfExperiments[experiment].physic_mean_value_dict_scaled_to_current
                value = []
                for i in range(len(sensorList)):
                    value.append(faulty[sensorList[i]] - reference[sensorList[i]])
                    valueArray = np.array(value)
                    print(np.linalg.norm(valueArray))
                self.differential_scaled_Mean_Values_without_Noise.append(value)
            else: pass
        # create figure
        fig, ax = plt.subplots(figsize=set_size())
        for experiment in range(len(self.differential_scaled_Mean_Values_without_Noise)):
            ax.plot(sensorList, self.differential_scaled_Mean_Values_without_Noise[experiment], marker = ".", label = listOfNames[experiment])
        ax.legend()
        plt.xticks(rotation = 90)
        plt.ylabel("B-Field ($\mu$T)")
        # get y-axis limits of the plot
    ...
    def plot_differential_Mean_without_Noise_with_Reference_to_input(self, experimentName:str) -> list:
        """Plot differntial value compared to the passed experiment"""
        # get mean values without noise for ref experiment:
        for experiment in range(self.nbrExperiments):
            if self.listOfExperiments[experiment].name == experimentName:
                reference = self.listOfExperiments[experiment].compute_Physic_Mean_minus_Noise_mean_Values()
                sensorList = self.listOfExperiments[experiment].sensorsOfInterest
            else: pass
        
        self.differential_Mean_without_Noise_with_Reference_to_input = []
        listOfNames =[]

        for experiment in range(self.nbrExperiments):
            if self.listOfExperiments[experiment].name != experimentName:
                listOfNames.append(self.listOfExperiments[experiment].name)
                faulty = self.listOfExperiments[experiment].compute_Physic_Mean_minus_Noise_mean_Values()
                value = []
                for i in range(len(sensorList)):
                    value.append(faulty[sensorList[i]] - reference[sensorList[i]])
                    valueArray = np.array(value)
                    print(np.linalg.norm(valueArray))
                self.differential_Mean_without_Noise_with_Reference_to_input.append(value)
            else: pass
        # create figure
        fig, ax = plt.subplots(figsize=set_size())

        for experiment in range(len(self.differential_Mean_without_Noise_with_Reference_to_input)):
            ax.plot(sensorList, self.differential_Mean_without_Noise_with_Reference_to_input[experiment], marker = ".", label = listOfNames[experiment])
        ax.legend()
        plt.xticks(rotation = 90)
        plt.ylabel("B-Field ($\mu$T)")
        # get y-axis limits of the plot
